...
# Special thanks to prepre the documentation: github.com/Azumi67
# Source: github.com/wangyu-/udp2raw
#
# This script is designed to simplify the installation and configuration of a
# Wireguard-based IPV6 tunnel using the UDP2RAW tool. It provides options to
# install required packages, configure the remote and local servers, and
# uninstall the configuration. The script detects your server's architecture to
# download the appropriate Jq and UDP2RAW binary.
#
...
#
# For more information and updates, visit [Your Website or GitHub Repository].
#
# Disclaimer:
...
remote_func() {
    ...
    # Read remote_port and validate it
    while true; do
        echo ""
...
if [ $remote_status -eq 0 ]; then
    echo -e "${CYAN}Server Status (KHAREJ)${NC} >>> ${GREEN}Wireguard Tunnel is running.${NC}"
else
    echo -e "${CYAN}Server Status (KHAREJ)${NC} >>> ${RED}Wireguard Tunnel is not running.${NC}"
fi

if [ $local_status -eq 0 ]; then
    echo -e "${CYAN}Client Status (IRAN)${NC} >>> ${GREEN}Wireguard Tunnel is running.${NC}"
else
    echo -e "${CYAN}Client Status (IRAN)${NC} >>> ${RED}Wireguard Tunnel is not running.${NC}"
fi
}

while true; do
    clear
    title_text="Wireguard IPV6 Tunnel"
    tg_title="TG-Group @OPIranCluB"
    yt_title="youtube.com/@opiran-inistitute"
    author="Author : Azumi & OPIran"
    
    echo ""
    echo -e "                 \e[35m$title_text\e[0m"
    echo -e "\e[33m______________________________________________________\e[0m"
    logo
    echo -e "\e[34m$tg_title\e[0m"
    echo -e "\e[34m$yt_title\e[0m"
    echo -e "   \e[92m$author\e[0m"
    echo -e "\e[33m______________________________________________________\e[0m"
    echo ""
    menu_status
    echo ""
    echo -e "\e[36m 1\e[0m) \e[32m \e[91m>>>> \e[92mInstall UDP2RAW binary\e[91m<<<<\e[0m"
    echo -e "\e[36m 2\e[0m) \e[93mConfigure Wireguard Tunnel on Remote (\e[93mKharej\e[93m)\e[0m"
    echo -e "\e[36m 3\e[0m) \e[93mConfigure Wireguard Tunnel on Local (\e[93mIran\e[93m)\e[0m"  
    echo ""
    echo -e "\e[36m 4\e[0m) \e[93m \e[31m >>>> \e[97mUninstall\e[31m<<<<\e[0m"
    echo -e "\e[36m \e[96m0\e[0m) \e[31m  >>>> \e[97mExit\e[31m<<<<\e[0m"
    echo ""
